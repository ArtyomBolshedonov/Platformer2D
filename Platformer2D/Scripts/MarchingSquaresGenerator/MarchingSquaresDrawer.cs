...
{
    internal sealed class MarchingSquaresDrawer
    {
        private SquareGrid _squareGrid;
        private Tilemap _tileMapGround;
        private Tile _tileGround;
        private Vector2 _mapStartPosition;

        public MarchingSquaresDrawer(Vector2 mapStartPosition)
        {
        ...
        private void DrawTileInControlNode(bool active, Vector3 position)
        {
            if (active)
            {
                var positionTile = new Vector3Int((int)(_mapStartPosition.x + position.x),
                    (int)(_mapStartPosition.y + position.y), 0);
