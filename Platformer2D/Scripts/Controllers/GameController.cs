...
using UnityEngine;
using CellAutomatGenerator;
using MarchingSquaresGenerator;
...
namespace Platformer2D
{
    internal sealed class GameController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            var cellAutomatGenerateLevelView = FindObjectOfType<CellAutomatGenerateLevelView>();
            var cellAutomatLevelGenerator = new CellAutomatLevelGenerator(cellAutomatGenerateLevelView);
            cellAutomatLevelGenerator.Generate();
            var marchingSquaresLevelView = FindObjectOfType<MarchingSquaresLevelView>();
            var marchingSquaresLevelGenerator = new MarchingSquaresLevelGenerator(marchingSquaresLevelView);
            marchingSquaresLevelGenerator.GenerateLevel();
        }
