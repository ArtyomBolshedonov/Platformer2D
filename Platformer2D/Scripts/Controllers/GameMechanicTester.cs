...
namespace Platformer2D
{
    internal sealed class GameMechanicTester : IExecute
    {
        ...
        public GameMechanicTester(GameStarter gameStarter)
        {
            ...
            foreach (var lever in _gameStarter.LeverObjectViews)
            {
                var singleQuestController = new SingleQuestController(lever);
                _quests.Add(singleQuestController.Quest);
            }
            var questStory = new QuestStory(_quests);
        }
